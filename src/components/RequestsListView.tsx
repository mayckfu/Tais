...
export const RequestsListView: React.FC<RequestsListViewProps> = ({
  ...
  return (
    <div id="requests-list-view" className="space-y-6">
      {/* Header */}
      <div className="bg-white rounded-xl shadow-xs border border-slate-200 p-4 sm:p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        <div>
        ...
        {/* View Layout Switcher & Sorting */}
        <div className="flex flex-wrap items-center gap-2 text-xs w-full lg:w-auto">
          {/* Layout Toggle: Cards vs Table */}
          <div className="flex items-center bg-[#F9F7F2] p-1 rounded-xl border border-[#E8E6D9]">
            <button
              ...
              title="Visualização em Cartões (Otimizada para Mobile e Toque)"
            >
            ...
            <button
              ...
              title="Visualização em Tabela Completa (Ideal para Telas Grandes)"
            >
          ...
          <div className="flex items-center gap-1 flex-1 sm:flex-initial">
            <span className="text-slate-500 font-medium shrink-0 hidden sm:inline">Ordenar:</span>
            <select
              value={sortField}
      ...
      {/* Requests Display: Mobile Protocol Cards OR Desktop Table */}
      {isCardsMode ? (
        <div id="requests-mobile-cards-list" className="space-y-3">
          {filteredRequests.length === 0 ? (
            ...
            filteredRequests.map((req) => (
              <MobileProtocolCard
                ...
                onOpenArrivalModal={(r) => {
                  const inTransit = r.relocations.find((m) => m.status === 'em_deslocamento');
                  if (inTransit && onOpenArrivalModal) {
                    onOpenArrivalModal(inTransit, r);
                  } else {
                    onOpenDetails(r);
                  }
                }}
